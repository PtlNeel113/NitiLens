...
from app.middleware.subscription_middleware import check_transaction_limit
...
def trigger_scan(
    ...
    """
    # Check transaction limit before scanning
    service = SubscriptionService(db)
    
    # Estimate transaction count (you can make this dynamic)
    estimated_transactions = 1000  # Adjust based on actual dataset size
    
    limit_check = service.check_transaction_limit(current_user.org_id, estimated_transactions)
    if not limit_check.get("allowed"):
        raise HTTPException(
            status_code=403,
            detail=limit_check.get("reason", "Monthly transaction limit exceeded. Upgrade your plan.")
        )
    ...
    service.increment_transaction_usage(current_user.org_id, len(violations))
