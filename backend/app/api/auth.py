...
from app.middleware.subscription_middleware import check_user_limit
...
class AddUserRequest(BaseModel):
    ...
    password: str
    full_name: str
    role: str = "compliance_analyst"


@router.post("/add-user")
def add_user_to_organization(
    user_data: AddUserRequest,
    current_user: User = Depends(check_user_limit),
    db: Session = Depends(get_db)
):
    """Add new user to current organization (requires user limit check)"""
    # Check if user exists
    existing_user = db.query(User).filter(User.email == user_data.email).first()
    if existing_user:
        raise HTTPException(status_code=400, detail="Email already registered")
    ...
    if current_user.role not in ["compliance_admin", "org_admin"]:
    ...
    service.update_user_count(current_user.org_id)
